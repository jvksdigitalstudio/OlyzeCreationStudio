...
object MeshRasterizer {
    ...
    /**
     * BUG REAL encontrado y corregido acá — mismo motivo exacto que en
     * [com.yeivikas.olyzecs.engine.effects.ImageEffects.blitBlend] y en
     * [com.yeivikas.olyzecs.engine.distortion.DistortionRasterizer]: promediar
     * 4 muestras premultiplicando por alfa (no antes), para que un texel
     * transparente vecino a uno opaco no manche con un negro "de mentira"
     * el borde de la textura sobre la malla 3D.
     */
    private fun downsample(src: IntArray, rw: Int, rh: Int, ss: Int, outW: Int, outH: Int): Bitmap {
        ...
        for (oy in 0 until outH) {
            ...
            for (ox in 0 until outW) {
                val sx0 = ox * ss
                var a = 0f; var rPm = 0f; var gPm = 0f; var bPm = 0f
                for (dy in 0 until ss) {
                    ...
                    for (dx in 0 until ss) {
                        ...
                        val af = ca / 255f
                        a += ca
                        // Premultiplicado: un sample transparente aporta 0.
                        rPm += ((c ushr 16) and 0xFF) * af
                        gPm += ((c ushr 8) and 0xFF) * af
                        bPm += (c and 0xFF) * af
                    }
                }
                ...
                if (ai <= 0) {
                    ...
                    val alphaAvgFraction = (a * norm) / 255f
                    val ri = ((rPm * norm) / alphaAvgFraction).toInt().coerceIn(0, 255)
                    val gi = ((gPm * norm) / alphaAvgFraction).toInt().coerceIn(0, 255)
                    val bi = ((bPm * norm) / alphaAvgFraction).toInt().coerceIn(0, 255)
                    out8 = (ai shl 24) or (ri shl 16) or (gi shl 8) or bi
                }
                out[oy * outW + ox] = out8
            }
        }
    ...
    /**
     * BUG REAL encontrado y corregido acá — mismo motivo exacto que en
     * [com.yeivikas.olyzecs.engine.distortion.DistortionRasterizer.sampleBilinearPixel]:
     * premultiplicar por alfa antes de interpolar, para que un texel
     * transparente vecino a uno opaco no oscurezca el borde de la
     * textura envuelta sobre la malla 3D con un negro "de mentira".
     */
    private fun sampleBilinear(texPixels: IntArray, texW: Int, texH: Int, u: Float, v: Float): Int {
        ...
        val r = (lerp(rTopPm, rBotPm, ty) / alphaFraction).toInt().coerceIn(0, 255)
        val g = (lerp(gTopPm, gBotPm, ty) / alphaFraction).toInt().coerceIn(0, 255)
        val bl = (lerp(bTopPm, bBotPm, ty) / alphaFraction).toInt().coerceIn(0, 255)
