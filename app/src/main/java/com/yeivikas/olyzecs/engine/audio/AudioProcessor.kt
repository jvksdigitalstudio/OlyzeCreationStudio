...
object AudioProcessor {
    ...
    internal fun buildProjectSamples(decoded: DecodedPcm, clip: AudioClip, projectDurationMs: Long): ShortArray {
        ...
        // en el frame 0 de salida; ahora los frames [0, startFrame) quedan
        // en silencio real (el ShortArray ya nace en 0, no hace falta
        // tocarlos) y la escritura real arranca en `startFrame`.
        ...
        val writableFrames = targetFrames - startFrame
        ...
        while (written < writableFrames) {
            ...
            val dstIndex = (startFrame + written) * channels
    ...
    internal fun applyVolumeAndFades(
        ...
        val baseVolume = volume.coerceIn(0f, 1.5f)
        if (baseVolume == 0f) {
            pcm.fill(0)
            return
        }
