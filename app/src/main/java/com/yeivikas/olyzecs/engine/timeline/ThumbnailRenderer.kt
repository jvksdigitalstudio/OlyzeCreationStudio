 ...
 *
 * FASE 4.3 (RENDER BOUNDARY, R4.3.7/R4.3.8): ahora implementa
 * [ThumbnailRenderCapability] — sus 2 únicos llamadores reales
 * (`TimelineApiImpl`, `ProjectStorage`) ya no dependen de ESTE `object`
 * concreto (que sí conoce EGL/GLES), sino de esa interfaz, con este
 * mismo objeto como valor por defecto. Comportamiento en tiempo de
 * ejecución sin cambios — es la misma implementación, la única que
 * existe hoy. Los valores por defecto de `timeMs`/`widthPx`/`heightPx`
 * se quitaron de ACÁ porque Kotlin no permite que un `override`
 * redeclare los defaults de la interfaz (quedan declarados una sola vez,
 * en [ThumbnailRenderCapability]) — sin impacto real: los 2 call sites
 * existentes siempre pasaban los 5 argumentos explícitos.
 */
object ThumbnailRenderer : ThumbnailRenderCapability {
