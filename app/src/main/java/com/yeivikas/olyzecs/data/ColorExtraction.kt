...
object ColorExtraction {
     ...
     * (mismo matiz, o acromático, o mismo cuadrante cuantizado según el
     * llamador) con su peso total y la suma de sus componentes RGB — de
     * acá sale el promedio real del balde ganador. Clase compartida entre
     * [extractFrom] (paleta de UN color dominante) y [extractPalette]
     * (paleta COMPLETA) — antes existían como dos clases privadas
     * (`Bucket`/`PaletteBucket`) idénticas byte a byte, código duplicado
     * real sin ninguna diferencia de comportamiento entre ambas.
     */
    private class WeightedColorBucket {
        var weight = 0.0
        var r = 0.0
        var g = 0.0
        var b = 0.0
    ...
    private fun extractFrom(pixels: IntArray): Int {
        ...
        // grande y apagado.
        val hueBuckets = Array(HUE_BUCKETS) { WeightedColorBucket() }
        ...
        val blackBucket = WeightedColorBucket()
        val grayBucket = WeightedColorBucket()
        val whiteBucket = WeightedColorBucket()
    ...
    fun extractPalette(source: Bitmap, maxColors: Int = 24, quantStep: Int = 24): List<Int> {
        ...
        return try {
            ...
            val buckets = HashMap<Long, WeightedColorBucket>()
            for (px in pixels) {
                val a = (px ushr 24) and 0xFF
                if (a < MIN_ALPHA) continue
                val r = (px ushr 16) and 0xFF
                ...
                val bucket = buckets.getOrPut(key) { WeightedColorBucket() }
