...
/**
 * Editor de encuadre genérico: muestra la imagen COMPLETA con la zona que
 * va a quedar fuera del marco oscurecida, y un recuadro claro en el
 * centro con la relación de aspecto exacta de [targetAspectRatio] — el
 * usuario arrastra para mover y pellizca para hacer zoom hasta centrar lo
 * que quiere que se vea, y recién ahí confirma. Nada se guarda hasta
 * tocar el botón de confirmación.
 *
 * Extraído de lo que originalmente era `CoverAdjustDialog`/`CoverAdjustContent`
 * (ver ADR-006): la lógica de gestos, overlay y recorte era idéntica para
 * cualquier relación de aspecto/resolución de salida — lo único que
 * cambiaba de un caso a otro (portada de proyecto vs. fondo de proyecto)
 * eran esos cuatro valores, ahora recibidos como parámetro en vez de
 * estar fijos en constantes de archivo. [CoverAdjustDialog] y
 * [BackgroundAdjustDialog] son wrappers delgados sobre este componente,
 * cada uno fijando sus propios valores — ningún llamador externo a
 * cualquiera de los dos necesita saber que comparten implementación.
 */
@Composable
fun ImageFitDialog(
    imageUri: Uri,
...
@Composable
private fun ImageFitError(onDismiss: () -> Unit) {
    Column(
 ...
 * overlay oscuro con "ventana" clara del tamaño exacto de [targetAspectRatio].
 * El tamaño del área de edición se captura una sola vez con
 * [onGloballyPositioned] y se guarda en [containerSize] — a partir de ahí,
 * el recuadro de encuadre (frameW/frameH) y la escala aplicada
 * (appliedScale) son valores normales de esta función, visibles tanto para
 * el [Canvas] que dibuja como para el botón de confirmación que recorta —
 * así lo que se guarda es EXACTAMENTE lo que el usuario vio dentro del
 * recuadro, ni un píxel distinto.
 */
@Composable
private fun ImageFitContent(
    bitmap: Bitmap,
    targetAspectRatio: Float,
    outputWidthPx: Int,
    outputHeightPx: Int,
    ...
    // recuadro de encuadre (equivalente a ContentScale.Crop); a partir de
    // ahí el usuario puede acercar más, nunca alejar por debajo de "cubre
    // completo" (si no, aparecerían bordes vacíos dentro del recuadro).
    ...
    // Recuadro de encuadre: 82% del ancho disponible, salvo que eso lo
    // haga demasiado alto para la pantalla — ahí se limita por alto y se
    ...
    var frameH = frameW / targetAspectRatio
        ...
        Box(
            ...
            if (hasSize) {
                Canvas(
                    ...
                    // recuadro de encuadre — así se ve claro y nítido solo
                    // lo que realmente va a quedar dentro, y oscuro/opaco
                    // todo lo demás de la imagen que no se va a ver.
                    ...
                    // perfectamente nítido dónde termina el encuadre.
                    drawRect(
                        color = Color.White,
...
/**
 * Recorta [bitmap] al rectángulo que el recuadro de encuadre está
 * mostrando en pantalla en este instante (dado su [appliedScale] y
 * desplazamiento [offsetX]/[offsetY] respecto al centro de la imagen), y
 * lo reescala a [outputWidthPx]×[outputHeightPx] — resolución exacta
 * pedida por quien llama a [ImageFitDialog] (fija y liviana para
 * portada, o la resolución completa del lienzo para fondo de proyecto),
 * sin depender del tamaño original de la foto elegida.
 */
private fun cropToOutputBitmap(
    ...
    val scaled = Bitmap.createScaledBitmap(
        cropped,
        outputWidthPx.coerceAtLeast(1),
        outputHeightPx.coerceAtLeast(1),
        true
    )
