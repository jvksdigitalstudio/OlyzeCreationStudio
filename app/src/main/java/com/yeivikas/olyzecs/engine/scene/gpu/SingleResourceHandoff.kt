 ...
 * prompt maestro de Fase 4.3): esta clase vivía antes en
 * `engine.render`, pero `engine.scene.Layer` la importaba desde ahí — es
 * decir, existía una dependencia real `scene → render` en el código
 * (además de la ya esperada `render → scene`, ya que `GLRenderer`
 * necesita conocer `Layer`), formando un ciclo de paquetes real:
 * `scene ↔ render`.
 *
 * Esta clase es, y siempre fue, COMPLETAMENTE genérica
 * (`SingleResourceHandoff<T : Any>`, sin ningún tipo de Android/GLES/
 * render) — su responsabilidad conceptual es un primitivo de OWNERSHIP
 * de un recurso (quién es dueño del último `Bitmap` decodificado
 * pendiente de subir a GPU) que pertenece al ESTADO de `Layer`/Scene, no
 * a la implementación del backend de render. `GLRenderer` la CONSUME (a
 * través de `Layer.pendingBitmap`), no la posee ni la define — la
 * dirección correcta de la dependencia es `render → scene.gpu`, nunca al
 * revés.
 *
 * Se movió acá, a un paquete propio (`engine.scene.gpu`, no directamente
 * `engine.scene`, para no mezclarla con los modelos de escena "puros"
 * como `Layer`/`LookSettings`, mientras sigue dejando claro que su
 * ownership conceptual es de Scene) para que `engine.scene` deje de
 * necesitar importar nada de `engine.render`. El comportamiento de la
 * clase NO cambió: mismo código, mismo contrato, mismos tests (ver
 * `SingleResourceHandoffTest`, movido junto con ella).
 *
