...
class VideoExporter(private val context: Context) {
    ...
    fun export(
        ...
        try {
            ...
            // AUDITORÍA — orden de liberación corregido: acá se liberaba
            // primero el `codec` (que es dueño del `Surface` de entrada
            // que el EGL de arriba envuelve como `eglSurface`) y RECIÉN
            // DESPUÉS se destruía ese `eglSurface`/`eglContext` — al
            // revés del orden que documentan tanto la guía de Android
            // para `MediaCodec.createInputSurface()` como las muestras de
            // referencia (p. ej. Grafika/bigflake `CodecInputSurface`):
            // el lado EGL que envuelve el `Surface` de un encoder debe
            // liberarse SIEMPRE antes de liberar el propio encoder/su
            // `Surface` subyacente — destruir una `EGLSurface` cuyo
            // `Surface`/BufferQueue productor ya fue liberado por
            // `codec.release()` es una condición "usar después de
            // liberar" a nivel de driver gráfico, con comportamiento no
            // garantizado entre fabricantes de GPU (en el mejor caso, un
            // log de advertencia; en el peor, un crash nativo que ni
            // siquiera este `try/catch` de Kotlin puede atrapar, porque
            // ocurre del lado de EGL/driver, no de la JVM). Se invierte
            // el orden: primero EGL, recién después el codec — sin
            // ningún cambio de comportamiento en el caso normal (ambos
            // ya estaban completamente drenados en ese punto), solo se
            // corrige la SECUENCIA de una limpieza que hasta ahora
            // dependía de que ningún fabricante de GPU fuera estricto al
            // respecto.
            releaseEgl()
