 ...
 *
 * [initialCanvas] (ver ADR-005, Fase E): el Canvas real elegido en el
 * `CreateProjectDialog` de [com.yeivikas.olyzecs.ui.ProjectsScreen]
 * (preset del catálogo o Custom). `null` = compatibilidad con cualquier
 * caller que todavía no lo pase, en cuyo caso `EditorViewModel` cae al
 * mismo puente de migración que usa para proyectos legacy.
 *
...
class EditorViewModelFactory(
    ...
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        return EditorViewModel(
            ...
            initialName, initialAspect, initialCanvas, initialDurationMs, initialFps, mesh3DApi, animationApi, distortionApi
        ) as T
    }
}
