...
class ProjectDataSerializationTest {
    ...
    @Test
    fun `un proyecto con Canvas real sobrevive intacto una vuelta completa de serializacion`() {
        val original = representativeProject().copy(
            canvasWidthPx = 1080,
            canvasHeightPx = 1920,
        ...
        val restored = json.decodeFromString<ProjectData>(json.encodeToString(original))
        assertEquals(original, restored)
        assertEquals(1080, restored.canvasWidthPx)
        assertEquals(1920, restored.canvasHeightPx)
        assertEquals("social.vertical.reels", restored.canvasOriginPresetId)
        assertEquals(1, restored.canvasSchemaVersion)
    }

    @Test
    fun `un Canvas Custom sin preset de origen tambien sobrevive la vuelta con originPresetId null`() {
        val original = representativeProject().copy(
            canvasWidthPx = 3000,
            canvasHeightPx = 1500,
            canvasOriginPresetId = null,
            canvasSchemaVersion = 1
        )
        val restored = json.decodeFromString<ProjectData>(json.encodeToString(original))
        assertEquals(original, restored)
        assertEquals(null, restored.canvasOriginPresetId)
    }

    @Test
    fun `un json viejo sin campos de Canvas carga con schemaVersion 0 y los 3 campos en null (senal de formato viejo)`() {
        // Simula project.json guardado ANTES de ADR-005 Fase C: no existen
        // canvasWidthPx/canvasHeightPx/canvasOriginPresetId/canvasSchemaVersion
        // todavía. `ProjectStorage.loadProject` usa exactamente esta
        // combinación (schemaVersion 0 + campos null) como señal para
        // migrar desde `aspectRatio` en vez de leer un Canvas a medias.
        val oldJson = """
            {
                "id": "old-project",
                "name": "Proyecto viejo",
        ...
        assertEquals("SQUARE", restored.aspectRatio)
        assertEquals(0, restored.canvasSchemaVersion)
        assertEquals(null, restored.canvasWidthPx)
        assertEquals(null, restored.canvasHeightPx)
        assertEquals(null, restored.canvasOriginPresetId)
    }
