...
- ✅ Eliminar capa (ícono de papelera) — ya NO borra nada al instante:
  la capa pasa a la papelera del proyecto, recuperable — ver "Fase 8 —
  papelera de capas" más abajo y `docs/fases/FASE_8_PAPELERA_DE_CAPAS.md`
...
## Fase 8 — completada: papelera de capas

Eliminar una capa ya no la destruye al instante. `EditorViewModel.removeLayer`
sigue sacándola del lienzo sin cambios, pero el próximo autoguardado —
comparando contra el `project.json` anterior — detecta la diferencia y la
mueve a una papelera persistida en el propio archivo del proyecto
(`ProjectData.trashedLayers`), con su imagen, keyframes y look intactos.
Ver `docs/fases/FASE_8_PAPELERA_DE_CAPAS.md` para el detalle técnico
completo (modelo de datos, las 5 operaciones nuevas de `ProjectStorage`,
todas las decisiones de diseño acordadas con el usuario, y los 2 hallazgos
de la auditoría posterior). Resumen:

- ✅ **Bloqueo de la última capa** — un proyecto ya no puede quedar con 0
  capas desde la UI de borrado normal; el diálogo avisa y ofrece la vía
  real (eliminar el proyecto entero desde "Mis proyectos").
- ✅ **Papelera sin límite de retención automático** — se acumula hasta
  que el usuario decide vaciarla, con el peso en disco siempre visible.
- ✅ **Aviso proactivo (banner)** — solo cuando el proyecto queda con
  exactamente 1 capa viva y hay algo en la papelera; una sola vez por
  episodio, nunca en cada apertura.
- ✅ **Pantalla de gestión con selección múltiple** — restaurar o eliminar
  las capas elegidas, o vaciar la papelera entera de un toque, cada una
  con su propia confirmación cuando corresponde.
- ✅ **Restaurar devuelve al lienzo de inmediato** (cierra la pantalla de
  la papelera); eliminar/vaciar no, para poder seguir revisando el resto.
- ✅ **Persistida de verdad** — sobrevive cerrar la app o que el sistema
  mate el proceso, con la misma escritura atómica y el mismo mutex por
  proyecto que ya protegen el resto de `project.json`.
...
### Opción C — GitHub Actions (flujo real de este proyecto)
Este repositorio se compila en CI vía `.github/workflows/android-build.yml`, que usa
`gradle/actions/setup-gradle` y llama a `gradle` directo (`assembleDebug`,
`testDebugUnitTest`) — a propósito, sin depender de `gradlew`/`gradle-wrapper.jar`,
que no están comiteados en el repo. El flujo normal es: editar/subir vía Termux →
push a la rama → Actions compila y deja los APKs como artifact.

### Opción D — Local con Gradle propio (opcional, no es el flujo principal)
Si en algún momento querés compilar localmente sin pasar por CI, necesitás generar
el wrapper vos mismo una vez (requiere una instalación de Gradle local):
```bash
...
│   │                             # + (FASE 3.1) SingleResourceHandoff.kt, TextureInvalidationRequest.kt
│   │                             # — cierre de ownership GPU/bitmap, ver docs/fases/FASE_3_1_CIERRE_OWNERSHIP.md
...
├── api/                         # EliNer API v1 — EliNerApi define 9 dominios (incluye
│                                 # `render: PixelColorSource`); 8 tienen implementación API
│                                 # conectable hoy, `render` sigue pendiente de composición
│                                 # productiva (ver su README.md, Fase 4.1)
├── data/
...
└── ui/
    ...
    ├── LayerTrashScreen.kt        # (Fase 8) Papelera de capas — banner + pantalla completa,
    │                               # ver docs/fases/FASE_8_PAPELERA_DE_CAPAS.md
...
`engine/render`, `engine/audio`, etc. — ver árbol de arriba). El paquete
Kotlin `api/` YA implementa el CONTRATO de `EliNer API v1` (interfaces +
implementaciones reales, ver `api/README.md`) — lo que falta para esta
arquitectura futura no es implementar la API, sino separar `engine`/`api`
en los módulos Gradle propios listados arriba (`eliner-api`,
`eliner-engine`, etc.), algo deliberadamente fuera de alcance hasta que
exista la migración a C++/JNI del roadmap. El plan
