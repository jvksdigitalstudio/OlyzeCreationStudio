 ...
 * versionado en este repositorio) sección 14 ("EliNer debe delegar
 * hacia el Engine, no reimplementar lógica").
 *
 * Uno de los dominios implementados de punta a punta ya en la Etapa 1
 * (ver `AnimationApiImpl`, junto con Mesh3D en ese momento): es
 * matemática pura, sin `Context`/Android, sin estado propio que
 * gestionar — cero riesgo de tocar UI/ViewModel/Engine para que exista
 ...
 * `MainActivity` (ver tarea "Animation → EliNer") — actualizado en
 * Fase 4.1: ya no es 1 de 2, sino uno de los 4 dominios (junto con
 * Export, Mesh3D y Distortion) que usan `EliNer API` de verdad, no solo
 * "conectada y sin usar" (ver KDoc de [com.yeivikas.olyzecs.api.EliNerApi]
 * para el estado completo de los 9 dominios).
 */
