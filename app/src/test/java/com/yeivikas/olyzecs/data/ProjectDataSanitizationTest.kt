...
class ProjectDataSanitizationTest {
    ...
    // --- Canvas real (ver ADR-005, Fase C) ---

    @Test
    fun `un Canvas real valido queda exactamente igual tras sanear`() {
        val valid = baseProject().copy(
            canvasWidthPx = 1080,
            canvasHeightPx = 1920,
            canvasOriginPresetId = "social.vertical.reels",
            canvasSchemaVersion = 1
        )
        assertEquals(valid, sanitizeProjectData(valid))
    }

    @Test
    fun `canvasWidthPx en cero o negativo hace caer todo el Canvas de vuelta a formato viejo`() {
        val cero = sanitizeProjectData(
            baseProject().copy(canvasWidthPx = 0, canvasHeightPx = 1920, canvasSchemaVersion = 1)
        )
        assertEquals(null, cero.canvasWidthPx)
        assertEquals(null, cero.canvasHeightPx)
        assertEquals(0, cero.canvasSchemaVersion)

        val negativo = sanitizeProjectData(
            baseProject().copy(canvasWidthPx = -1080, canvasHeightPx = 1920, canvasSchemaVersion = 1)
        )
        assertEquals(null, negativo.canvasWidthPx)
        assertEquals(0, negativo.canvasSchemaVersion)
    }

    @Test
    fun `canvasHeightPx invalido tambien invalida el ancho, un Canvas no puede quedar a medias`() {
        val sanitized = sanitizeProjectData(
            baseProject().copy(canvasWidthPx = 1080, canvasHeightPx = -5, canvasSchemaVersion = 1)
        )
        assertEquals(null, sanitized.canvasWidthPx)
        assertEquals(null, sanitized.canvasHeightPx)
        assertEquals(0, sanitized.canvasSchemaVersion)
    }

    @Test
    fun `un Canvas invalidado tambien pierde su originPresetId, no queda huerfano apuntando a dimensiones descartadas`() {
        val sanitized = sanitizeProjectData(
            baseProject().copy(
                canvasWidthPx = 0,
                canvasHeightPx = 1920,
        ...
        assertEquals(null, sanitized.canvasOriginPresetId)
    }

    // --- AUDITORÍA (hallazgo corregido): el rango también tiene límite SUPERIOR ---

    @Test
    fun `un canvasWidthPx absurdamente grande tambien hace caer el Canvas de vuelta a formato viejo`() {
        // Antes de este fix, sanitizeProjectData solo exigia "> 0" — un
        // project.json corrupto o editado a mano con, por ejemplo,
        // canvasWidthPx = 999999999 pasaba el saneo intacto (CanvasSpec.init
        // no rechaza valores grandes, solo los <= 0), llegando después como
        // Canvas real a hitTestBoxSize/GLPreview/al encoder de exportación.
        val gigante = sanitizeProjectData(
            baseProject().copy(
                canvasWidthPx = CanvasLimits.MAX_DIMENSION_PX + 1,
                canvasHeightPx = 1920,
                canvasOriginPresetId = "social.vertical.reels",
                canvasSchemaVersion = 1
            )
        )
        assertEquals(null, gigante.canvasWidthPx)
        assertEquals(null, gigante.canvasHeightPx)
        assertEquals(null, gigante.canvasOriginPresetId)
        assertEquals(0, gigante.canvasSchemaVersion)
    }

    @Test
    fun `los limites exactos de CanvasLimits (min y max) se consideran validos, no se descartan por error de borde`() {
        val minimo = sanitizeProjectData(
            baseProject().copy(
                canvasWidthPx = CanvasLimits.MIN_DIMENSION_PX,
                canvasHeightPx = CanvasLimits.MIN_DIMENSION_PX,
                canvasSchemaVersion = 1
            )
        )
        assertEquals(CanvasLimits.MIN_DIMENSION_PX, minimo.canvasWidthPx)
        assertEquals(1, minimo.canvasSchemaVersion)

        val maximo = sanitizeProjectData(
            baseProject().copy(
                canvasWidthPx = CanvasLimits.MAX_DIMENSION_PX,
                canvasHeightPx = CanvasLimits.MAX_DIMENSION_PX,
                canvasSchemaVersion = 1
            )
        )
        assertEquals(CanvasLimits.MAX_DIMENSION_PX, maximo.canvasWidthPx)
        assertEquals(1, maximo.canvasSchemaVersion)
    }
