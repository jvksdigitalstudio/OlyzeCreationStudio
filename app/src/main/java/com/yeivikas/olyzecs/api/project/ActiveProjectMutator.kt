 ...
 *
 * No almacena estado, no contiene lógica de ownership. Implementada por
 * `EditorViewModel`; cada `*ApiImpl` de escritura la recibe por
 * constructor.
 *
 * FASE 4.2 — CIERRE TÉCNICO R5 (R5.8 del prompt maestro de cierre R5):
 * la frase de arriba decía, sin matices, "sin `Context`" — impreciso: dos
 * métodos de este mismo archivo ([previewPlayFrom]/[previewSeekTo], ver
 * más abajo) SÍ reciben `Context` como parámetro (nunca almacenado,
 * permitido explícitamente por ADR-002 — ver la nota "TERCERA
 * CORRECCIÓN" más abajo, que ya explicaba el porqué desde que se agregó,
 * solo que el resumen de arriba no reflejaba esa excepción). La regla
 * real de esta interfaz es: **sin `Context` almacenado como campo, sin
 * ownership de recursos que dependan de él** — recibirlo por parámetro,
 * puntualmente, cuando una operación de UI (como controlar el
 * `MediaPlayer` de preview) genuinamente lo necesita, está permitido.
 * Se corrige acá la imprecisión de la documentación para que coincida
 * con el código real, sin cambiar ningún comportamiento (R5.19 del
 * prompt de cierre R5: "no cambies código solamente para satisfacer una
 * frase incorrecta de documentación").
 *
