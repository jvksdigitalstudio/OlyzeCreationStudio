 ...
 * un campo `@Transient` de bitmap CPU en tránsito (`pendingBitmap`) que no
 * tiene sentido persistir — es estado de render en memoria, no datos del
 * proyecto (el texture id GL, desde la Fase 3.1, ni siquiera es un campo
 * de `Layer` — vive en un registro privado de `GLRenderer`, así que no
 * hay nada que excluir a mano acá para ese caso). `imageFileName` reemplaza al Uri original: es el nombre del
...
data class AudioTrackData(
    ...
    // Agregados junto con la reactivación de la fila de audio en el
    // timeline (ver AudioTrackRow/TimelineView) — con default explícito
    // para que un proyecto guardado ANTES de este cambio siga cargando
    // sin romperse: `trackOrder = 0f` lo deja al tope de la playlist tal
    // como se veía siempre (audio nunca existía como fila propia antes),
    // y `timelineStartMs = 0L` preserva el comportamiento de siempre
    // (arrancar en t=0 del proyecto).
    val trackOrder: Float = 0f,
    val timelineStartMs: Long = 0L
)

/**
 * Snapshot de UNA capa que el usuario eliminó del lienzo, conservado en la
 * papelera del proyecto — no en un archivo/directorio aparte: la capa
 * conserva [layer] tal cual estaba en el momento de borrarla (imagen,
 * keyframes, look, transform, todo) más [deletedAtMs] para poder mostrar
 * "hace cuánto" y, a futuro, cualquier política de limpieza que se agregue.
 *
 * Reutiliza [LayerData] completo en vez de un subconjunto de campos a
 * propósito: restaurar una capa de la papelera tiene que devolverla
 * EXACTAMENTE como estaba, no una versión "liviana" que perdió sus
 * keyframes de cámara o su look — la papelera es un lugar de espera, no
 * una versión degradada de la capa.
 *
 * `imageFileName` (dentro de [layer]) sigue apuntando al mismo archivo en
 * `images/` de siempre: una capa en papelera NO mueve su imagen a ningún
 * otro directorio, solo dejar de estar en [ProjectData.layers] y pasar a
 * [ProjectData.trashedLayers] ya alcanza para que `ProjectStorage` sepa
 * que ese archivo sigue siendo válido y no debe borrarse en la limpieza de
 * huérfanos post-guardado (ver el comentario grande sobre `validImageNames`
 * en `ProjectStorage.saveProject`).
 */
@Serializable
...
data class ProjectData(
    ...
    // --- Canvas real del proyecto (ver ADR-005, Fase C) ---
    // `aspectRatio` de arriba queda deprecado, conservado SOLO para la
    // ruta de migración: un proyecto guardado ANTES de esta mejora no
    // tiene ninguno de los 4 campos de acá abajo en su project.json (los
    // 3 `Int?`/`String?` se leen como `null` y `canvasSchemaVersion` cae
    // a su default `0`) — `canvasSchemaVersion == 0` es justamente la
    // señal de "formato viejo, todavía no migrado" que usa
    // `ProjectStorage.loadProject` para decidir si migra desde
    // `aspectRatio` (ver `LegacyAspectMigration`) o si usa estos 3
    // campos directo. Desde esta versión, CADA guardado (incluso el
    // autoguardado normal del editor) escribe siempre los 4 campos con
    // `canvasSchemaVersion = 1`, así que un proyecto solo pasa por la
    // migración la primera vez que se vuelve a guardar después de
    // abrirse.
    val canvasWidthPx: Int? = null,
    val canvasHeightPx: Int? = null,
    // Puramente informativo (ver CanvasSpec.originPresetId) — nunca una
    // dependencia funcional. Null tanto en proyectos viejos como en un
    // Canvas Custom que no vino de ningún preset del catálogo.
    val canvasOriginPresetId: String? = null,
    // 0 = formato viejo (solo `aspectRatio`, sin Canvas propio, ver
    // arriba). 1 = este proyecto ya tiene `canvasWidthPx`/`canvasHeightPx`
    // reales y confiables, no hace falta migrar nada al cargarlo.
    val canvasSchemaVersion: Int = 0,
    val fps: Int = 30,
    ...
    // BUG REAL corregido: `playheadMs` SIEMPRE se recibía como parámetro
    // de `ProjectStorage.saveProject` (se usaba para renderizar la
    // miniatura con el frame que se estaba viendo al guardar), pero nunca
    // se escribía a este modelo ni se leía de vuelta al reabrir — por eso
    // el proyecto siempre volvía a arrancar desde el segundo 0 del
    // timeline en vez de continuar donde el usuario lo había dejado.
    // Default 0L: un proyecto guardado ANTES de este campo simplemente
    // arranca desde el principio, igual que se comportaba hasta ahora.
    val playheadMs: Long = 0L,
    // --- Papelera de capas ---
    // Capas eliminadas del lienzo, conservadas para poder arrepentirse —
    // ver [DeletedLayerData] y el comentario grande de "Papelera de capas"
    // en ProjectStorage.saveProject (dónde se agregan) y en
    // ProjectStorage.restoreLayersFromTrash/deleteLayersFromTrashPermanently/
    // emptyTrash (dónde se sacan). SIN límite de retención automático a
    // propósito (decisión de producto): se acumulan hasta que el usuario
    // mismo decide vaciar la papelera — a cambio, la UI siempre muestra el
    // tamaño en disco que ocupa, para que esa decisión sea informada, no a
    // ciegas (ver LayerTrashScreen.kt).
    val trashedLayers: List<DeletedLayerData> = emptyList(),
    // true = ya se le avisó al usuario, en algún momento, que este proyecto
    // tiene capas en la papelera esperando revisión (ver
    // EditorUiState.showTrashRecoveryBanner) — no se lo vuelve a interrumpir
    // con el mismo aviso mientras siga siendo la MISMA situación. Se
    // reinicia a `false` automáticamente la próxima vez que una capa NUEVA
    // cae a la papelera (ver ProjectStorage.saveProject): un episodio nuevo
    // de "cayó algo a la papelera" merece un aviso nuevo, no reutilizar
    // el "ya lo vi" de un episodio anterior. Default `true`: un proyecto
    // guardado ANTES de esta función nunca tuvo nada en su papelera, así
    // que no hay nada que avisar de todos modos — este default es solo
    // para que la ausencia del campo en un project.json viejo nunca se
    // interprete, por error, como "hay que avisar".
    val trashBannerAcknowledged: Boolean = true
)

/**
 * Metadata persistida junto a un proyecto movido a la papelera de
 * proyectos (ver [ProjectStorage.moveProjectToTrash]) — vive en un
 * archivo JSON HERMANO de la carpeta movida
 * (`projects_trash/<projectId>.trash_meta.json`), nunca DENTRO de esa
 * carpeta: el navegador de archivos de la papelera
 * ([ProjectStorage.listTrashedProjectEntries]) tiene que mostrar el
 * contenido REAL del proyecto tal cual estaba (`project.json`, `images/`,
 * `audio/`, `cast/`, miniatura, portada) sin un archivo ajeno de esta app
 * mezclado en el medio — exactamente el pedido explícito de diseño ("se
 * puede ver la carpeta principal del proyecto... y dentro de esa carpeta
 * todo en subcarpetas organizado").
 *
 * `name` se captura en el momento de mover a la papelera (copia del
 * `ProjectData.name` de ese instante) para poder listar la papelera aun
 * si, por algún motivo, `project.json` dentro de la carpeta movida
 * terminara corrupto o ilegible — la lista de la papelera nunca debe
 * quedar en blanco solo porque el archivo interno no se pudo parsear.
 */
@Serializable
data class ProjectTrashMeta(
