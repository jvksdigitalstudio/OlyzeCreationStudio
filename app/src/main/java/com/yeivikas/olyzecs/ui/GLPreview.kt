...
fun GLPreview(
    ...
    // raíz completa. Este callback llega desde el hilo de GL — se
    // reenvía al hilo principal con `Handler(Looper.getMainLooper())`
    // antes de invocarlo, porque quien lo conecta (EditorScreen) va a
    // tocar un `MutableState` de Compose, y Compose solo es seguro desde
    // el hilo principal.
    ...
    // El callback de GLRenderer llega desde el hilo de GL — este Handler
    // (creado una sola vez, sobre el Looper principal) es lo que permite
    // saltar de vuelta al hilo de UI antes de tocar el Compose State real.
        ...
        factory = { context ->
            GLSurfaceView(context).apply {
                ...
                val renderer = GLRenderer(
                    ...
                    // pasar por `rememberUpdatedState` (ver el comentario
                    // en el parámetro de la función): el `factory` de
                ...
                renderer.onAllVisibleLayersPainted = {
                    mainHandler.post { currentOnAllVisibleLayersPainted() }
                }
