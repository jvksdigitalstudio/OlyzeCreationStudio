...
object ImageEffects {
    ...
    /**
     * Radio de difuminado en píxeles, proporcional al lado mayor de la imagen (5%).
     *
     * BUG REAL encontrado y corregido acá (auditoría, reporte del usuario:
     * "Difuminado de la sombra" al 100% se veía casi sin difuminar en
     * imágenes grandes): el techo de [coerceIn] estaba fijo en 48px
     * absolutos sin importar la resolución real de la capa. Esta función es
     * compartida por CASI todos los efectos (sombra, sombra de relleno,
     * resplandor, grosor/difuminado de contorno, sombra de contacto,
     * reflejo y el desenfoque de fondo), así que ese techo los afectaba a
     * todos por igual: en una imagen decodificada a resolución real (varios
     * miles de píxeles de ancho, ver [ImageDecoding.NO_LIMIT]), el 5%
     * calculado (150-200px+) se aplastaba SIEMPRE a 48px — un slider al
     * 100% terminaba produciendo el mismo difuminado visual que uno al
     * ~25-30%, y la sombra "leía" como un recorte casi duro en vez de una
     * sombra difusa. [boxBlur] es O(ancho×alto) por pasada sin importar el
     * radio (usa sumas corridas / sliding window, no un kernel que crezca
     * con el radio), así que subir este techo no tiene costo de
     * performance adicional relevante — se sube a 5% real hasta un techo
     * mucho más alto (320px) que sí sigue evitando radios absurdos en
     * lienzos extremos, en vez de un techo pensado para imágenes chicas.
     */
    private fun blurRadiusPx(amount: Float, width: Int, height: Int): Int {
        val maxRadius = (max(width, height) * 0.05f).coerceIn(2f, 320f)
     ...
     */
    /**
     * Interpolación bilineal de 4 texeles vecinos alrededor de ([fx],[fy])
     * (coordenadas de píxel, no de centro — el llamador ya restó 0.5).
     * Coordenadas fuera de rango se clampean al borde válido más cercano
     * en vez de tratarse como transparente — correcto acá porque
     * [blitBlend] solo la usa sobre capas ya difuminadas/con padding
     * (sombra, relleno, contacto), donde el alfa real ya cae a ~0 antes
     * de llegar al borde del bitmap; clampear evita un halo oscuro
     * adicional en el borde que SÍ aparecería si se tratara como negro
     * transparente puro.
     *
     * Se pre-multiplica RGB por A antes de promediar (y se divide de
     * vuelta al final) — promediar RGB sin premultiplicar entre un texel
     * opaco y uno transparente typeable "tira" el color hacia el RGB de
     * relleno del píxel transparente (que en este archivo no tiene
     * significado real), oscureciendo/decolorando el borde exactamente
     * en la zona donde más se nota. Mismo motivo por el que [boxBlur]
     * documenta trabajar sobre valores sin premultiplicar para SU caso
     * (ahí los 4 canales son independientes por diseño); acá, al
     * mezclar solo 4 muestras para un resample, premultiplicar es lo
     * correcto.
     */
    private fun sampleBilinear(pixels: IntArray, w: Int, h: Int, fx: Float, fy: Float): Int {
        val x0 = floor(fx).toInt()
        val y0 = floor(fy).toInt()
        val tx = fx - x0
        val ty = fy - y0
        val x0c = x0.coerceIn(0, w - 1)
        val x1c = (x0 + 1).coerceIn(0, w - 1)
        val y0c = y0.coerceIn(0, h - 1)
        val y1c = (y0 + 1).coerceIn(0, h - 1)

        val p00 = pixels[y0c * w + x0c]
        val p10 = pixels[y0c * w + x1c]
        val p01 = pixels[y1c * w + x0c]
        val p11 = pixels[y1c * w + x1c]

        fun premulComponents(p: Int): FloatArray {
            val a = (p ushr 24) and 0xFF
            val af = a / 255f
            val r = ((p ushr 16) and 0xFF) * af
            val g = ((p ushr 8) and 0xFF) * af
            val b = (p and 0xFF) * af
            return floatArrayOf(a.toFloat(), r, g, b)
        }

        val c00 = premulComponents(p00)
        val c10 = premulComponents(p10)
        val c01 = premulComponents(p01)
        val c11 = premulComponents(p11)

        val out = FloatArray(4)
        for (i in 0..3) {
            val top = c00[i] + (c10[i] - c00[i]) * tx
            val bottom = c01[i] + (c11[i] - c01[i]) * tx
            out[i] = top + (bottom - top) * ty
        }

        val outA = out[0].roundToInt().coerceIn(0, 255)
        if (outA == 0) return 0
        val invA = 255f / outA
        val outR = (out[1] * invA).roundToInt().coerceIn(0, 255)
        val outG = (out[2] * invA).roundToInt().coerceIn(0, 255)
        val outB = (out[3] * invA).roundToInt().coerceIn(0, 255)
    ...
    private fun blitBlend(
        ...
        for (dy in startY until endY) {
            for (dx in startX until endX) {
                ...
                // Coordenada fuente en punto flotante (sin truncar a
                // entero): [sampleBilinear] interpola entre los 4 texeles
                // vecinos en vez de tomar el más cercano.
                //
                // BUG REAL encontrado y corregido acá (auditoría): antes
                // esto truncaba a `.toInt()` y hacía un lookup nearest-
                // neighbor directo — sobre una capa SIN transformar (sin
                // escala/inclinación/perspectiva) es invisible, pero en
                // cuanto [ImageEffectsParams.shadowScale] != 1,
                // [shadowSkewDegrees] != 0 o [shadowPerspectiveAmount] > 0
                // (los mismos controles que arma [compositeWithEffects]
                // para la sombra/relleno/contacto vía este mismo
                // [blitBlend]) la matriz inversa ya no cae en coordenadas
                // enteras, y el nearest-neighbor produce un borde
                // "escalonado"/dentado sobre lo que ya es, de por sí, un
                // borde previamente difuminado suave — un defecto de
                // calidad visible sobre todo en inclinaciones/escalas
                // pronunciadas. La interpolación bilineal es el estándar
                // de cualquier motor de compositing para este tipo de
                // resample con transformación arbitraria.
                val fx = srcPt[0] - 0.5f
                val fy = srcPt[1] - 0.5f
                if (fx < -1f || fx > srcW.toFloat() || fy < -1f || fy > srcH.toFloat()) continue
                val sp = sampleBilinear(srcPixels, srcW, srcH, fx, fy)
