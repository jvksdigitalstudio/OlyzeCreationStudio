...
class QuadProjectionTest {
    ...
    // ---- Boundary hardening (FASE 4.3 — auditoría Render Math Boundary) ----
    //
    // Los casos de abajo no reproducen ningún bug reportado — son la
    // cobertura de contrato que la auditoría de esta intervención pidió
    // explícitamente para las dos únicas funciones matemáticas puras que
    // UI y Render comparten como fuente única de verdad (viewport
    // inválido, geometría degenerada, dato de cámara corrupto). Fijan por
    // regresión el comportamiento defensivo que el código YA tenía antes
    // de esta pasada (guarda de entrada en `projectQuadCornersNdc`,
    // chequeo de determinante en `mapPointToUv`) — no se modificó
    // producción para que estos tests pasen.

    @Test
    fun `projectQuadCornersNdc devuelve null con viewport o imagen invalidos`() {
        // imageWidthPx/imageHeightPx en 0 o negativo: no hay geometría de
        // capa que proyectar. viewportWidthPx/viewportHeightPx en 0 o
        // negativo: no hay lienzo donde encajar el resultado. Cualquiera
        // de los cuatro, solo o combinado, debe abortar sin excepción.
        assertEquals(null, PerspectiveCameraMath.projectQuadCornersNdc(
            translateX = 0f, translateY = 0f, scaleVal = 1f, scaleXVal = 1f, scaleYVal = 1f,
            rotationDeg = 0f, tiltXDeg = 0f, tiltYDeg = 0f, parallaxFactor = 1f,
            dollyZoom = 0f, dollyRange = dollyRange, baseEyeZ = baseEyeZ,
            imageWidthPx = 0, imageHeightPx = 1000, viewportWidthPx = 1000f, viewportHeightPx = 1000f
        ))
        assertEquals(null, PerspectiveCameraMath.projectQuadCornersNdc(
            translateX = 0f, translateY = 0f, scaleVal = 1f, scaleXVal = 1f, scaleYVal = 1f,
            rotationDeg = 0f, tiltXDeg = 0f, tiltYDeg = 0f, parallaxFactor = 1f,
            dollyZoom = 0f, dollyRange = dollyRange, baseEyeZ = baseEyeZ,
            imageWidthPx = 1000, imageHeightPx = -5, viewportWidthPx = 1000f, viewportHeightPx = 1000f
        ))
        assertEquals(null, PerspectiveCameraMath.projectQuadCornersNdc(
            translateX = 0f, translateY = 0f, scaleVal = 1f, scaleXVal = 1f, scaleYVal = 1f,
            rotationDeg = 0f, tiltXDeg = 0f, tiltYDeg = 0f, parallaxFactor = 1f,
            dollyZoom = 0f, dollyRange = dollyRange, baseEyeZ = baseEyeZ,
            imageWidthPx = 1000, imageHeightPx = 1000, viewportWidthPx = 0f, viewportHeightPx = 1000f
        ))
        assertEquals(null, PerspectiveCameraMath.projectQuadCornersNdc(
            translateX = 0f, translateY = 0f, scaleVal = 1f, scaleXVal = 1f, scaleYVal = 1f,
            rotationDeg = 0f, tiltXDeg = 0f, tiltYDeg = 0f, parallaxFactor = 1f,
            dollyZoom = 0f, dollyRange = dollyRange, baseEyeZ = baseEyeZ,
            imageWidthPx = 1000, imageHeightPx = 1000, viewportWidthPx = 1000f, viewportHeightPx = -1f
        ))
    }

    @Test
    fun `projectQuadCornersNdc con dollyZoom fuera del rango -1 a 1 de la UI no produce NaN ni Infinity`() {
        // La UI acota dollyZoom a [-1, 1] (LabeledSlider en EditorScreen),
        // pero CameraFrame/Keyframe.dollyZoom NO se sanitiza al cargar un
        // proyecto desde disco (a diferencia de los campos de Grid, que sí
        // se acotan explícitamente en ProjectStorage — ver
        // FASE_4_3_RENDER_BOUNDARY.md). Un JSON de proyecto editado a mano
        // o corrompido podría entregar un dollyZoom fuera de ese rango.
        // Este test documenta y fija que, aun así, la cámara sigue siendo
        // matemáticamente estable: `eyeZ` nunca llega a 0 salvo un valor
        // exacto de dollyZoom (~-1.5625) fuera de cualquier rango realista,
        // y `atan` está acotado por construcción — no hay división por
        // cero real en el dominio de valores que puede producir la app.
        for (extremeDollyZoom in listOf(-1f, 1f, -3f, 5f, -1.5f)) {
            val corners = project(parallaxFactor = 0.35f, dollyZoom = extremeDollyZoom)
            assertNotNull("viewport válido: nunca debería dar null", corners)
            for (corner in corners!!) {
                for (component in corner) {
                    assertTrue(
                        "dollyZoom=$extremeDollyZoom produjo un valor no finito: ${corner.toList()}",
                        component.isFinite()
                    )
                }
            }
    ...
    @Test
    fun `QuadHomography con las 4 esquinas coincidentes (cuadrilatero degenerado) no invierte`() {
        // Área cero: geométricamente no existe un mapeo UV único hacia ese
        // punto. `fromUnitSquare` no debe lanzar excepción (debe caer en
        // la rama de denominador ~0 ya existente para paralelogramos) y
        // `mapPointToUv` debe reportar la imposibilidad devolviendo null
        // (determinante ~0), nunca un resultado numérico arbitrario.
        val p = floatArrayOf(3f, 7f)
        val coeffs = QuadHomography.fromUnitSquare(p, p, p, p)
        val uv = QuadHomography.mapPointToUv(coeffs, p[0], p[1])
        assertEquals(null, uv)
    }

    @Test
    fun `QuadHomography con un cuadrilatero casi degenerado (sliver) sigue siendo invertible`() {
        // Caso "extremo pero válido" simétrico al anterior: área
        // pequeñísima pero no cero. No debe degradarse a null (eso sería
        // sub-inclusivo y rompería el hit-test en capas muy comprimidas
        // por scaleY) siempre que el determinante siga siendo
        // numéricamente distinguible de cero.
        val tl = floatArrayOf(-1f, 0.001f); val tr = floatArrayOf(1f, 0.001f)
        val br = floatArrayOf(1f, -0.001f); val bl = floatArrayOf(-1f, -0.001f)
        val coeffs = QuadHomography.fromUnitSquare(tl, tr, br, bl)
        val center = QuadHomography.mapUvToPoint(coeffs, 0.5f, 0.5f)
        val backToUv = QuadHomography.mapPointToUv(coeffs, center[0], center[1])
        assertNotNull(backToUv)
        assertEquals(0.5f, backToUv!![0], 1e-2f)
        assertEquals(0.5f, backToUv[1], 1e-2f)
    }
}
