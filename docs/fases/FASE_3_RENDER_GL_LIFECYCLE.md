...
> **⚠️ Corrección posterior (FASE 3.1):** una auditoría del código real
> (no solo de este documento) encontró que las afirmaciones de las
> secciones §15 y §19 sobre `Layer.glTextureId`/`Layer.pendingBitmap`
> ("ya eran correctas", "GL thread ownership", "sin cambios") NO se
> correspondían con lo que el código realmente permitía: el hilo
> principal (`EditorViewModel`) escribía `layer.glTextureId` directamente
> en varios lugares, y `pendingBitmap` tenía una ventana de carrera real
> (lost update) entre lectura y limpieza. Ambas secciones fueron
> corregidas más abajo para reflejar el estado real encontrado y el
> arreglo aplicado — ver el informe completo en
> `docs/fases/FASE_3_1_CIERRE_OWNERSHIP.md`.
...
tal como existía hasta esta fase) **sí** se invalidaban correctamente
(`= -1` en cada `onSurfaceCreated`, código ya presente antes de esta
fase) — aunque, como encontró la auditoría de cierre 3.1, ese mismo campo
tenía un problema de ownership distinto (el hilo principal también lo
escribía directamente; ver `FASE_3_1_CIERRE_OWNERSHIP.md`) que esta
sección de Fase 3 no llegó a detectar. Resultado observable de ESTE bug
(§3.2): la
...
- **Texturas de capa (`Layer.glTextureId`, tal como existía hasta esta
  fase)**: el mecanismo de invalidación en `onSurfaceCreated` (reset a
  `-1` incondicional para cada capa vista, forzando re-decodificación
  desde `Layer.sourceUri`/`Layer.pendingBitmap`) era correcto — **pero la
  afirmación de que el campo en sí tenía "GL thread ownership" NO era
  cierta**: la auditoría de cierre 3.1 encontró que `EditorViewModel`
  también escribía `layer.glTextureId` directamente en varios lugares
  (`replaceLayer`, `restoreSnapshot`, `discardChangesAndExit`). Corregido
  en la Fase 3.1: el campo ya no existe en `Layer` — el texture id GL
  vive exclusivamente en un registro privado de `GLRenderer`, y el hilo
  principal solo puede pedir invalidación (nunca escribir el handle). Ver
  `FASE_3_1_CIERRE_OWNERSHIP.md`.
- **Textura de la cuadrícula (`gridTextureCache`)**: corregida en esta
  fase, ver §3.2/§5. La Fase 3.1 encontró y corrigió, además, un bug
  distinto en el mismo método (`recycle()` sobre un bitmap que Compose/UI
  seguía considerando de su propiedad) — ver el informe de cierre.
...
> **Corregido en Fase 3.1** (ver banner al principio de este documento):
> la fila "Texturas de capa" de abajo describía la mecánica de
> invalidación correctamente, pero la columna de ownership real era
> falsa — el hilo principal SÍ podía (y lo hacía) escribir
> `Layer.glTextureId` directamente. La tabla actualizada, reflejando el
> registro por capa que ahora vive en `GLRenderer` y ya no en `Layer`,
> está en `FASE_3_1_CIERRE_OWNERSHIP.md` (tabla de ownership obligatoria,
> §20 de ese informe).

| Recurso | Creado por | Destruido por | Recreable | Fuente de verdad persistente |
|---|---|---|---|---|
| Texturas de capa (histórico, hasta Fase 3.1) | `GLRenderer.uploadTextureIfNeeded` (hilo GL) | `LayerDrawer.deleteTexture` (hilo GL), en `onSurfaceCreated` si `glTextureId>=0` antes de resetear, o al reemplazar por una nueva | Sí (ya lo era antes de esta fase) | `Layer.sourceUri` / `Layer.pendingBitmap` (CPU) — **ownership del campo `glTextureId` en sí NO estaba garantizada, ver corrección de Fase 3.1** |
