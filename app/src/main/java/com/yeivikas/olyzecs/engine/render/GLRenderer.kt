...
import com.yeivikas.olyzecs.engine.scene.gpu.SingleResourceHandoff
...
class GLRenderer(
    ...
    // DIAGNÓSTICO TEMPORAL (14/sep/2026) — throttle del log
    // "DIAG_BOX_VS_RENDER" (ver el mismo tag en EditorScreen.kt, en el
    // bloque que dibuja el marco morado de selección). Corre en el hilo
    // de GL, que dibuja a ~60fps — sin este throttle, inundaría logcat.
    //
    // BUG DE INSTRUMENTACIÓN corregido (14/sep/2026, sesión de diagnóstico
    // real con el usuario): este throttle era UN SOLO Long compartido
    // entre TODAS las capas del proyecto, no uno por capa. El bucle de
    // `onDrawFrame` recorre las capas en orden de `zIndex` y, apenas
    // cualquiera de ellas pasaba el chequeo de 800ms, consumía el único
    // timer disponible — así que, en un proyecto con fondo + capa
    // seleccionada, el fondo (case normal: dibujado primero, zIndex más
    // bajo) "ganaba" el throttle casi siempre, y el [RENDER] de la capa
    // que el usuario realmente tiene seleccionada/tocada — la única que
    // importa para comparar contra el [OVERLAY] del marco — casi nunca
    // llegaba a loguearse. Resultado real observado en un registro de
    // errores capturado: de ~40 líneas [RENDER] en 55s, solo 4
    // correspondían a la capa seleccionada, y coincidir una de esas 4
    // con una línea [OVERLAY] de la MISMA capa en el MISMO instante fue
    // cuestión de suerte de timing, no algo garantizado. Corrección:
    // timer por capa (`MutableMap<String, Long>`, clave = layer id) para
    // que cada capa tenga su propia ventana de 800ms independiente — y,
    // más importante, la capa con un override en vivo (la que el usuario
    // está tocando/arrastrando AHORA, ver `override` más abajo) se loguea
    // SIEMPRE, sin pasar por el throttle, porque es la única que hace
    // falta comparar pixel a pixel contra el [OVERLAY] para encontrar una
    // desincronización real.
    private val lastBoxDiagLogAtMsByLayer = mutableMapOf<String, Long>()
    ...
    // incluso cuando termina cargando bien": `EditorViewModel.isLoadingProject`
    // pasa a `false` apenas `ProjectStorage.loadProject()` termina de
    // decodificar los bitmaps en CPU (ver ese archivo) — eso NO significa
    // que ya estén subidos a la GPU ni dibujados. `uploadTextureIfNeeded`
    // sube una textura por capa por frame de forma no bloqueante, y
    // `uploadTexture` en sí hace un `glFinish()` real (ver LayerDrawer) que,
    // con una imagen de fondo pesada (resolución completa del lienzo),
    // puede tardar uno o varios frames. EditorScreen ocultaba su overlay de
    // carga (fondo verde + spinner, ver `isLoadingProject` en EditorScreen.kt)
    // en el instante equivocado: al terminar el decode, no al terminar de
    // PINTAR. Durante esa ventana — típicamente milisegundos, pero
    // perceptible — quedaba expuesto el `glClearColor` verde de "lienzo
    // vacío" (intencional para chroma key, ver `ensureInitialized`) porque
    // la textura real todavía no estaba lista.
    //
    // Este callback avisa al hilo de UI (vía `onAllVisibleLayersPainted`,
    // conectado desde GLPreview/EditorScreen) recién cuando TODAS las capas
    // visibles del snapshot actual ya tienen una textura GL válida y se
    // dibujaron de verdad — el momento correcto para que EditorScreen recién
    // ahí retire el overlay verde de "cargando". Se dispara una sola vez por
    // creación de superficie (mismo criterio que `hasLoggedFirstFrame`, con
    // el que comparte el punto de reset en `onSurfaceCreated`) — si después
    // cambian las capas (agregar/quitar), EditorScreen ya no depende de esto
    // para mostrarlas: este aviso es solo para la carga inicial del proyecto.
    private var hasSignaledAllLayersPainted = false

    /**
     * Callback opcional, invocado UNA vez desde el hilo de GL cuando todas
     * las capas visibles del snapshot vigente en ese momento ya tienen una
     * textura GL válida y se dibujaron. Quien lo conecta (ver GLPreview.kt)
     * es responsable de saltar al hilo principal antes de tocar Compose —
     * este callback corre en el hilo de render de GLSurfaceView, nunca en
     * el de UI.
     */
    var onAllVisibleLayersPainted: (() -> Unit)? = null
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        hasSignaledAllLayersPainted = false
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        // Ver KDoc de [hasSignaledAllLayersPainted]/[onAllVisibleLayersPainted]:
        // arranca en `true` y se apaga apenas se detecta, dentro del `for`
        // de abajo, una capa visible cuya textura todavía no está lista —
        // así al salir del bucle indica sin ambigüedad si el frame que se
        // acaba de dibujar mostró de verdad TODAS las capas visibles, o si
        // alguna todavía está subiendo a GPU.
        var allVisibleLayersPaintedThisFrame = true
        ...
        for (snapLayer in orderedSnapshotLayers) {
            ...
            if (snapLayer.visible && liveTextureId < 0) {
                // Esta capa DEBERÍA verse pero todavía no tiene textura GL
                // (decode/upload en curso) — el frame de este ciclo no
                // cuenta como "todas las capas pintadas" todavía.
                allVisibleLayersPaintedThisFrame = false
            }
            ...
            // DIAGNÓSTICO TEMPORAL — ver el KDoc completo del mismo tag
            // "DIAG_BOX_VS_RENDER" del lado de EditorScreen.kt. Mismo
            // criterio de throttle (800ms) para no inundar logcat durante
            // reproducción/pellizco. Se loguea la capa SELECCIONADA
            // (identificable comparando el nombre con el log [OVERLAY])
            // — en la práctica solo hace falta comparar la capa que el
            // usuario tiene tocada en pantalla en ese instante.
            //
            // Throttle por capa (no global — ver el porqué en el KDoc de
            // `lastBoxDiagLogAtMsByLayer` más arriba) + bypass total del
            // throttle para la capa con override en vivo: esa es siempre
            // la capa seleccionada/tocada, la única que hace falta
            // comparar contra [OVERLAY], así que se loguea en TODOS los
            // frames mientras el usuario la esté moviendo — necesitamos
            // muestras durante el arrastre, no solo en reposo.
            val nowForBoxDiag = System.currentTimeMillis()
            val isLiveOverrideLayer = override != null && override.first == snapLayer.id
            val lastLogForThisLayer = lastBoxDiagLogAtMsByLayer[snapLayer.id] ?: 0L
            if (isLiveOverrideLayer || nowForBoxDiag - lastLogForThisLayer >= 800L) {
                lastBoxDiagLogAtMsByLayer[snapLayer.id] = nowForBoxDiag
                AppLogger.i(
                    "DIAG_BOX_VS_RENDER",
                    "[RENDER]  layer='${liveLayer.name}' live=$isLiveOverrideLayer " +
                        "imagePx=${liveLayer.widthPx}x${liveLayer.heightPx} " +
                        "viewportPx=${viewportWidth}x${viewportHeight} " +
                        "translate=(${frame.translateX},${frame.translateY}) scale=${frame.scale} " +
                        "scaleXY=(${frame.scaleX},${frame.scaleY}) rot=${frame.rotationDeg} " +
                        "tilt=(${frame.tiltXDeg},${frame.tiltYDeg}) dolly=${frame.dollyZoom} " +
                        "parallax=${snapLayer.parallaxFactor}"
                )
            }
            drawer.drawLayer(
        ...
        // Recién ACÁ —después de intentar subir y dibujar cada capa, no
        // antes— es el momento correcto para avisar "ya se pintó todo lo
        // que se veía al abrir el proyecto". Ver KDoc de
        // [hasSignaledAllLayersPainted]/[onAllVisibleLayersPainted].
        if (!hasSignaledAllLayersPainted && allVisibleLayersPaintedThisFrame) {
            hasSignaledAllLayersPainted = true
            onAllVisibleLayersPainted?.invoke()
        }
