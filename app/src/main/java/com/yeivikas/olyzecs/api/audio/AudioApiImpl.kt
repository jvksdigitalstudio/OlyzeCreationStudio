...
class AudioApiImpl(
    ...
    override suspend fun setAudioClip(sourceUri: Uri): Boolean {
        ...
        withContext(Dispatchers.Main.immediate) {
            // A PEDIDO EXPLÍCITO DEL USUARIO: el audio importado por esta
            // vía (EliNer API) tiene que respetar el mismo orden de carga
            // que el flujo manual de la UI (`EditorViewModel.importAudio`)
            // — nunca aparecer delante de capas ya existentes. Antes, acá
            // se dejaba el `trackOrder` en su default de clase (`0f`),
            // ajeno a cuántas capas ya había cargadas; se corrige usando
            // [nextAudioTrackOrder], la ÚNICA función que calcula este
            // valor (ver su KDoc en `AudioClip.kt`), con `reader.getLayers()`
            // — el mismo snapshot de capas que ya usa [getAudioClip] arriba
            // para leer el estado del proyecto activo.
            val existingClip = reader.getAudioClip()
            mutator.setAudioClipDirect(
                AudioClip(
                    sourceUri = sourceUri,
                    displayName = displayName,
                    sourceDurationMs = durationMs,
                    trackOrder = existingClip?.trackOrder ?: nextAudioTrackOrder(reader.getLayers())
                )
            )
        }
        return true
