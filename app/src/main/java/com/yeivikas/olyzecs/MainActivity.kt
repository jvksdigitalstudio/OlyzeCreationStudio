...
import com.yeivikas.olyzecs.viewmodel.ProjectsTrashViewModelFactory
            ...
            OlyzeTheme {
                ...
                Surface(
                    ...
                    if (projectId == null) {
                        ...
                        val projectsTrashViewModel: ProjectsTrashViewModel =
                            viewModel(factory = ProjectsTrashViewModelFactory(projectStorage))
                        ProjectsScreen(
                        ...
                        // FASE 10 — el editor necesita un ViewModelStoreOwner
                        // PROPIO por visita al proyecto, en vez de heredar el
                        // de MainActivity (`LocalViewModelStoreOwner.current`
                        // por defecto acá seguiría siendo la Activity, cuyo
                        // ViewModelStore vive TODO el proceso).
                        //
                        // BUG REAL corregido acá (reporte del cliente,
                        // reproducido paso a paso: "Guardar y salir" → volver
                        // a "Mis proyectos" → reabrir ESE MISMO proyecto sin
                        // cerrar la app → el mensaje ámbar de la papelera y
                        // su ícono NO aparecen aunque sí había una capa
                        // recién eliminada; cerrando y reabriendo la app
                        // entera sí aparecen). Causa raíz: `viewModel(factory
                        // = factory, key = projectId)`, unas líneas más
                        // abajo, busca-o-crea dentro del ViewModelStoreOwner
                        // que esté vigente en la composición — que sin este
                        // cambio es la Activity. La Activity vive todo el
                        // proceso, así que ese `EditorViewModel` quedaba
                        // CACHEADO ahí para siempre: al volver a abrir el
                        // mismo proyecto, `viewModel()` encontraba la
                        // instancia vieja por su key y la devolvía tal cual
                        // — su bloque `init` (que llama a
                        // `ProjectStorage.loadProject` y ahí computa
                        // `showTrashRecoveryBanner`/`trashedLayers` desde el
                        // disco, ver FASE 8-R3) NUNCA volvía a correr. Solo
                        // matar el proceso (que sí destruye el
                        // ViewModelStore de la Activity) forzaba una
                        // instancia nueva — de ahí que el cliente solo lo
                        // viera funcionar cerrando la app entera. Este
                        // comentario, antes en `EditorViewModel.onCleared()`,
                        // ya documentaba el síntoma de fondo sin corregirlo:
                        // "la app cachea el ViewModel por projectId mientras
                        // se navega... onCleared() normalmente solo corre
                        // cuando el proceso realmente termina" — spoiler:
                        // ese comentario ya no es cierto después de este
                        // cambio, y se actualizó ahí también.
                        //
                        // La corrección es el mismo patrón que usa
                        // Navigation Compose para cada `NavBackStackEntry`
                        // (cada entrada de la pila tiene su PROPIO
                        // ViewModelStore, creado al entrar y `clear()`-eado —
                        // lo que dispara `onCleared()` en cascada sobre todo
                        // lo que contenga — al salir de la pila): acá,
                        // `remember(projectId)` crea un `ViewModelStore`
                        // nuevo cada vez que se entra a este `else` para
                        // este `projectId` (volver a "Mis proyectos" saca
                        // esta rama de la composición por completo, así que
                        // `remember` lo olvida; reabrir el mismo proyecto
                        // vuelve a entrar de cero), y el
                        // `DisposableEffect` de abajo lo `clear()`-ea justo
                        // cuando esa composición se abandona — antes de que
                        // `viewModel()` pueda volver a reusar nada de la
                        // sesión anterior. `CompositionLocalProvider`
                        // publica este owner nuevo para TODO lo que cuelga
                        // de acá abajo (incluida la línea de `viewModel(...)`
                        // más abajo, que sigue exactamente igual — no hacía
                        // falta tocarla), sin afectar a `ProjectsScreen` ni a
                        // ningún otro `viewModel()` de fuera de esta rama,
                        // que se siguen resolviendo contra la Activity como
                        // siempre.
                        ...
                        // Se llama cada vez que se (re)entra a ESTE projectId.
                        // FASE 10: antes esto hacía falta porque el ViewModel
                        // podía venir reciclado del ViewModelStore de la
                        // Activity; ahora cada entrada al editor ya recibe una
                        // instancia NUEVA (ver
                        // FASE_10_VIEWMODEL_SCOPE_POR_SESION.md), cuyo `init`
                        // ya lee el estado vigente desde disco por su cuenta.
                        // Se deja la llamada igual, como red de seguridad
                        // idempotente — ver el KDoc actualizado de
                        // `resetPlaybackState()`/`refreshProjectNameFromDisk()`
                        // en EditorViewModel.kt para el detalle completo.
                        ...
                        } // cierra CompositionLocalProvider(LocalViewModelStoreOwner provides sessionViewModelStoreOwner)
                    }
                }
            }
        }
