...
private const val TAG = "LayerDrawer"
...
class LayerDrawer {
    ...
    // gran angular); más grandes = más sutil (teleobjetivo). Vive en
    // [PerspectiveCameraMath.BASE_EYE_Z] — ÚNICA fuente de verdad,
    // compartida con el overlay 2D de selección en EditorScreen.kt (ver
    // el KDoc de esa constante para el bug real que esto previene).
    private val baseEyeZ = PerspectiveCameraMath.BASE_EYE_Z
    // Cuánto se mueve físicamente la cámara con dollyZoom=±1.
    // Fuente de verdad: PerspectiveCameraMath.DOLLY_RANGE (ver ese archivo).
     ...
     *
     * BUG REAL encontrado en la auditoría de esta fase: [GLRenderer]
     * llamaba a [ensureInitialized] (no a este método) desde
     * `onSurfaceCreated()`, y `ensureInitialized` solo construye el
     * shader si `shaderProgram == null`. Esa condición es correcta la
     * PRIMERA vez que se crea el contexto EGL, pero `onSurfaceCreated`
     * se vuelve a llamar cada vez que Android le entrega al renderer un
     * contexto EGL NUEVO (recreación tras pérdida de contexto, volver de
     * segundo plano, reabrir un proyecto) — y en ese caso `shaderProgram`
     * seguía sin ser null (la misma instancia de [LayerDrawer] sobrevive,
     * ver comentario de clase más arriba), así que `ensureInitialized`
     * se saltaba la reconstrucción del shader por completo. El
     * `programId` que quedaba en memoria pertenecía al contexto EGL
     * VIEJO ya destruido — usarlo contra el contexto nuevo es, según el
     * driver, un no-op silencioso (nada se dibuja) o un id que por pura
     * casualidad coincide con otro recurso real del contexto nuevo
     * (dibujo corrupto). Ningún camino es el correcto.
     *
     * Este método reemplaza a `ensureInitialized()` en el ÚNICO lugar
     * donde corresponde invalidar sin condición (`GLRenderer.onSurfaceCreated`):
     * descarta la referencia al `ShaderProgram` viejo (SIN llamar a
     * `glDeleteProgram` sobre su id — pertenece a un contexto ya muerto,
     * mismo razonamiento que [GridTextureCacheState.invalidateForNewContext])
     * y reconstruye el shader desde cero bajo el contexto actual, que sí
     * está garantizado activo (contrato de `onSurfaceCreated`).
     */
    ...
    /**
     * Construye el shader SOLO si todavía no existe uno para esta
     * instancia. Pensado como red de seguridad defensiva dentro de
     * [drawLayer] (por si algo dibuja antes de que `onSurfaceCreated`
     * haya corrido) — la invalidación real ante un contexto EGL nuevo la
     * hace [forceReinitialize], no este método. Debe llamarse una vez
     * que el contexto EGL actual esté activo (current).
     */
    fun ensureInitialized() {
        if (shaderProgram == null) {
            ...
            // Solo se conserva la instancia si el shader realmente quedó
            // listo (ver el KDoc de ShaderProgram.build): si falla, se
            // deja `shaderProgram` en null a propósito, para que
            // `drawLayer` (que ya chequea `shaderProgram ?: return`)
            // siga saltando el dibujo en vez de intentarlo con un
            // `programId` inválido — y para que un `onSurfaceCreated`
            // posterior (nuevo contexto EGL) tenga otra oportunidad real
            // de reconstruirlo, en vez de quedar con un `shaderProgram`
            // no-null que `ensureInitialized` ya no volvería a tocar.
            if (candidate.build()) {
                shaderProgram = candidate
            } else {
                AppLogger.e(TAG, "El shader no quedó listo — esta capa no se va a dibujar hasta el próximo intento de inicialización")
            }
        }
    }
    ...
    fun drawLayer(
        ...
        //
        // Los escalares de esta sección (eyeZ, fovyDeg, depthZ,
        // depthCompensation) viven en [PerspectiveCameraMath] — extraídos
        // de acá para poder testearlos sin depender de
        // android.opengl.Matrix/GLES20 (no disponibles en tests JVM sin
        // Robolectric). Ver PerspectiveCameraMathTest, que fija por
        // regresión el bug de ADR-007 (verde chroma-key asomando en
        // fondos en reposo) simulando la proyección de perspectiva
        // completa con la misma matemática que Matrix.perspectiveM/
        // setLookAtM.
        val eyeZ = PerspectiveCameraMath.eyeZ(baseEyeZ, frame.dollyZoom, dollyRange)
        ...
        // por desplazamiento.
        val depthZ = PerspectiveCameraMath.depthZ(parallaxFactor, maxDepth = PerspectiveCameraMath.MAX_DEPTH)

        // --- Compensación de profundidad (fix del verde chroma-key
        // asomando en fondos — ver ADR-007) ---
        // El fitScale de más abajo (fitScaleX/fitScaleY) está calibrado
        // para que un plano en z=0 llene el viewport exacto — es la
        // garantía de "cero regresión" documentada más arriba. Pero
        // cualquier capa con parallaxFactor < 1 vive en z=depthZ < 0, MÁS
        // LEJOS de la cámara, así que sin corrección se proyecta más
        // chica que el viewport aunque su bitmap cubra el 100% del
        // lienzo por construcción (ver BackgroundAdjustDialog/ADR-006,
        // y ADR-010 para la segunda puerta de entrada al mismo síntoma
        // que ADR-006 no llegó a cerrar) — el hueco restante deja ver el
        // verde de fondo del canvas.
        //
        // La corrección: escalar hacia arriba en proporción exacta a
        // cuánto más lejos está esta capa que el plano de foco, medido
        // contra baseEyeZ (NO contra el eyeZ animado por dollyZoom) —
        // así, en reposo (dollyZoom=0), CUALQUIER capa llena el cuadro
        // sin importar su parallaxFactor, y el dolly zoom sigue
        // funcionando exactamente igual que antes: al animar dollyZoom,
        // eyeZ real se aparta de baseEyeZ y esta capa SÍ cambia de
        // tamaño relativo al sujeto (z=0) — el warp de perspectiva
        // buscado a propósito, sin tocar ese comportamiento.
        //
        // En z=0 (parallaxFactor=1, "sujeto"): depthZ=0 →
        // depthCompensation = baseEyeZ/baseEyeZ = 1 → sin cambio alguno,
        // preserva el "cero regresión" ya documentado arriba.
