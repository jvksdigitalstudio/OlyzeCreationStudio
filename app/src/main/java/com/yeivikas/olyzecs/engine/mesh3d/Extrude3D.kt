...
object Extrude3D {
    ...
    // BUG REAL encontrado y corregido acá: crash reproducible al mover
    // CUALQUIER slider de este efecto (registro real: OutOfMemoryError
    // en MeshRasterizer.kt:69, "Failed to allocate a 613708720 byte
    // allocation" ≈ 585MB en un solo array, para un buffer de ~12.386px
    // de lado — la app se cierra sola a los ~500ms de soltar el dedo,
    // que es justo el debounce del commit de calidad final más abajo).
    //
    // Causa raíz: [outW]/[outH] (el lienzo que rasteriza
    // [MeshRasterizer]) se armaban directo a partir de `source.width`/
    // `source.height` — es decir, la resolución COMPLETA de la foto tal
    // cual se decodificó para el commit final (sin límite de tamaño,
    // ver ImageDecoding.NO_LIMIT en EditorScreen). La fórmula
    // `outW = srcW + margin*2` con `margin = refRadius*1.35` ya de por
    // sí amplifica el lado largo de la foto por ~2.35x ANTES de aplicar
    // el supersampling (hasta 3x en calidad final) — o sea que el lienzo
    // rasterizado crece como ~7x el lado de la foto original, y
    // [MeshRasterizer.render] reserva DOS arrays de ese tamaño al
    // cuadrado (`pixels: IntArray` + `depthBuf: FloatArray`, 8 bytes por
    // píxel entre las dos). Para cualquier foto de cámara normal (ya ni
    // hablar de una de varios miles de píxeles de lado), el lienzo
    // resultante pesa cientos de MB a más de un GB — garantizado
    // OutOfMemoryError, no un caso raro/límite.
    //
    // Esto además viola la única garantía real que necesita este
    // efecto: la malla 3D no necesita ni un pixel más de detalle
    // GEOMÉTRICO que el que ya usa [MASK_MAX_SIDE_FINAL]/
    // [MASK_MAX_SIDE_LIVE] para trazar el contorno del silueteado (ya
    // limitado a 360/240px porque no hace falta más para una silueta
    // fiel) — el problema es que ese límite SOLO se aplicaba al trazado
    // del contorno, no a la cámara/margen/lienzo del resto de la
    // geometría, que seguía atada 1:1 al tamaño real de la foto.
    //
    // Arreglo: la geometría entera (radio de referencia, cámara, margen,
    // tapas, paredes y el propio lienzo de salida) ahora se arma sobre
    // una copia de trabajo escalada al mismo límite [maskMaxSide] que ya
    // regía el trazado del contorno — así el lienzo rasterizado queda
    // acotado a un tamaño seguro (~50MB en el peor caso de calidad
    // final, contra los +600MB de antes) SIN IMPORTAR la resolución de
    // la foto original. La textura de verdad (la que se ve, con todo su
    // detalle/nitidez) sigue siendo la foto real — ver [TEXTURE_MAX_LONG_SIDE]
    // más abajo, que la acota por separado con un margen mucho más
    // generoso, solo para blindar contra fotos de cámara enormes (12-48
    // megapíxeles) que también podrían agotar memoria en la copia cruda
    // de píxeles que arma [MeshRasterizer] para muestrear la textura —
    // un tope totalmente independiente del de la geometría, para no
    // perder nitidez visible por el camino.
    private const val TEXTURE_MAX_LONG_SIDE = 1600
    ...
    fun render(source: Bitmap, params: Params, highQuality: Boolean = true): Bitmap {
        val realSrcW = source.width
        val realSrcH = source.height
        if (realSrcW <= 0 || realSrcH <= 0) return source
        ...
        // Copia de trabajo para TODA la geometría (radio, cámara,
        // margen, tapas, paredes, lienzo de salida), acotada al mismo
        // límite [maskMaxSide] que ya regía el trazado del contorno —
        // ver el comentario grande junto a [TEXTURE_MAX_LONG_SIDE] más
        // arriba. Si la foto ya es más chica que el límite, no se toca
        // nada (mismo comportamiento de siempre para imágenes chicas).
        val geomScale = min(1f, maskMaxSide.toFloat() / max(realSrcW, realSrcH))
        val srcW = if (geomScale < 1f) max(1, (realSrcW * geomScale).roundToInt()) else realSrcW
        val srcH = if (geomScale < 1f) max(1, (realSrcH * geomScale).roundToInt()) else realSrcH
        val geomSource = if (geomScale < 1f) {
            runCatching { Bitmap.createScaledBitmap(source, srcW, srcH, true) }.getOrNull() ?: source
        } else source

        // Textura real para el color por píxel: se acota por separado
        // (con un margen mucho más generoso que el de la geometría,
        // ver [TEXTURE_MAX_LONG_SIDE]) solo para blindar contra fotos
        // de cámara enormes — no comparte el límite de la geometría
        // para no perder nitidez visible de más.
        val texScale = min(1f, TEXTURE_MAX_LONG_SIDE.toFloat() / max(realSrcW, realSrcH))
        val texture = if (texScale < 1f) {
            val texW = max(1, (realSrcW * texScale).roundToInt())
            val texH = max(1, (realSrcH * texScale).roundToInt())
            runCatching { Bitmap.createScaledBitmap(source, texW, texH, true) }.getOrNull() ?: source
        } else source
        ...
        buildWalls(
            mesh,
            traceAlphaBoundaries(geomSource, maskMaxSide),
        ...
        val result = MeshRasterizer.render(
            mesh, camera, texture, outW, outH, originX, originY, alpha255,
            supersample = supersample
        )
        if (geomSource !== source) geomSource.recycle()
        if (texture !== source && texture !== geomSource) texture.recycle()
        return result
    }
        ...
        for (cy in -1 until mh) {
            for (cx in -1 until mw) {
                ...
                when (code) {
                    ...
                    // BUG REAL encontrado y corregido acá: los dos casos
                    // "silla de montar" (esquinas opacas en diagonal, code
                    // 5 = bl+tr opacas / code 10 = tl+br opacas) tenían sus
                    // segmentos INTERCAMBIADOS entre sí.
                    //
                    // Cada segmento de este switch sigue una convención de
                    // sentido fija (verificable con el producto cruz entre
                    // la dirección del segmento y el vector hacia la
                    // esquina opaca que "aísla": siempre da +0.25, nunca
                    // -0.25 — se puede comprobar con las 4 plantillas de
                    // esquina única: bl-opaca=Seg(left,bottom) [code 1],
                    // br-opaca=Seg(bottom,right) [code 2],
                    // tr-opaca=Seg(right,top) [code 4],
                    // tl-opaca=Seg(top,left) [code 8]).
                    //
                    // Un caso "silla" no es más que DOS esquinas opacas
                    // diagonales resueltas como dos aislamientos
                    // independientes — code 5 (bl+tr opacas) tiene que
                    // reusar EXACTAMENTE las plantillas de bl [code 1] y
                    // tr [code 4]: Seg(left,bottom) + Seg(right,top). Pero
                    // el código traía las plantillas de tl [code 8] y br
                    // [code 2] — las de la diagonal CONTRARIA (la que en
                    // code 5 está transparente, no opaca). Exactamente al
                    // revés: code 10 (tl+br opacas) traía las plantillas
                    // de bl/tr en vez de las de tl/br.
                    //
                    // Efecto práctico: en cualquier recorte con un borde
                    // en diagonal de un solo píxel de ancho (moneda de
                    // pan, bordes en ~45°, dedos/patas finas — extremadamente
                    // común en un PNG recortado real), el contorno
                    // trazado en esa celda apuntaba al lado transparente
                    // en vez de al opaco. Como el encadenado de segmentos
                    // en loops (más abajo) empalma por coordenada exacta
                    // de inicio/fin, un segmento con el sentido equivocado
                    // no encaja con el segmento del vecino que sí viene
                    // bien orientado — el lazo se corta ahí, produciendo
                    // un contorno roto/incompleto (y por lo tanto una
                    // malla 3D con paredes agujereadas o picos) exactamente
                    // en los bordes diagonales finos de la silueta, el
                    // caso más común de todos en un personaje/objeto
                    // recortado real.
                    5 -> { segs.add(Seg(left, bottom)); segs.add(Seg(right, top)) }
                    6 -> segs.add(Seg(bottom, top))
                    7 -> segs.add(Seg(left, top))
                    8 -> segs.add(Seg(top, left))
                    9 -> segs.add(Seg(top, bottom))
                    10 -> { segs.add(Seg(top, left)); segs.add(Seg(bottom, right)) }
